...
We see that the data is organized in an _"untidy"_ format or _"wide"_ format. To make our analysis workflow more smoother we need to convert this data frame into the _"long"_ format. We do this by using the `gather()` function in the `tidyr` package. For more information on the concept of "tidy data" Hadley's paper can be found [here](https://www.jstatsoft.org/article/view/v059i10/v59i10.pdf).
...
Now let's change the `rank` variable into a numeric format.
...
In regards to actually detecting and replacing the values automatically I wasn't very successful. For each year, I can do this: 
```{r}
GPI_rank %>% 
...
I'm not going to bother manually changing all of them especially since there are a lot of NAs in the earlier years where many countries that show up in the later editions of the GPI had not been included yet. I'm sure there's a way to automate this using the above function and `lapply()`/`map()` to separate each year (to be able to better identify the problem rows) and then re-combine it all in one dataframe, I'll edit this section when I find out how to do it.
...
There could still be some improvements made to this plot, specifically, the country labels on either side of the x-axis limits. Using the `gg_repel` package the labels will be spread out better with a corresponding line pointing to the right line and point. We can manually set colors to each of our custom "East Asia" region countries to further differntiate each line and make it more easy to read:
...
change the variable type of `country` into a factor and then rename Korea Republic to S.Korea to fit graph labels better:
...
Of course, the changes in rankings aren't all due to inter-country conflicts as it is only 1 of the 22 indicators used to calcualte the GPI score. 
...
To do the same thing as creating a separate dataset (like `GPI_Asia`) is to simply create a new variable called `region` inside our existing `GPI_rank` data frame. 
...
GPI_rank %>%
  ...
  geom_label_repel(
    data = GPI_rank %>% 
      ...
      aes(label = country, x = "2008"), 
      color = "black", fontface = "bold", family = "Times",
      size = 4, nudge_x = -0.9) +
  geom_label_repel(
    data = GPI_rank %>% 
      filter(year == "2017", region == "East Asia"), 
      aes(label = country, x = "2017"), 
      color = "black", fontface = "bold", family = "Verdana",
      size = 4, nudge_x = 0.9) +
  scale_y_reverse(breaks = c(1, 5, seq(10, 160, by = 10))) +
  scale_x_discrete(expand = c(0.2, 0.05)) +
  labs(x = "Year", y = "Rank") +
